...
class ParseStrace(Thread):
    def __init__(self, pid=None, program=None, profile={}, callbackWarning=None, debug=False, loglevel=1):
        ...
        self.syscallsDicPattern={
            "execve"     : r".*\(\"(\S+)\",.*",
            "open"       : r".*\(\"(\S+)\",.*(O_RDONLY|O_RDWR|O_WRONLY).*",
            "openat"     : r".*\(AT_.*,\s*\"(\S+)\",.*(O_RDONLY|O_RDWR|O_WRONLY).*",
            "rename"     : r".*\(\"(\S+)\",\s*\"(\S+)\"",
            "socket"     : r".*(SOCK_DGRAM|SOCK_RAW|SOCK_STREAM|SOCK_SEQPACKET).*",
            "connect"    : None,
            "accept"     : None,
            "sendto"     : None,
            "recvfrom"   : None,
            "sendmsg"    : r".*family=AF_INET.*",
            "recvmsg"    : r".*family=AF_INET.*",
            "bind"       : r".*family=AF_INET.*",
            "listen"     : None,
            "socketpair" : None,
            "accept4"    : None,
            "sendmmsg"   : None
